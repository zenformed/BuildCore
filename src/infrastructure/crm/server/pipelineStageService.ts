...
export async function createOrganizationPipelineStage(
  ...
  const slug = ensureUniquePipelineStageSlug(slugifyPipelineStageLabel(trimmedLabel), takenSlugs);
  const nextSortOrder =
...
export async function deleteOrganizationPipelineStage(
  ...
  if (isReservedPipelineStageSlug(target.slug)) {
    throw new Error('This workflow stage is required by BuildCore and cannot be deleted.');
  }
  if (stages.length <= 1) {
...
export async function reorderOrganizationPipelineStages(
  ...
  await normalizeOrganizationPipelineStageSortOrder(supabase, organizationId, orderedStageIds);
  return buildBuildCorePipelineStagesResponse(supabase, organizationId, userId);
}
