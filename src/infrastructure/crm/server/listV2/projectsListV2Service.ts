...
async function callRootPageRpc(
  ...
  return ((data ?? []) as RpcPageRow[]).map((row) => ({
    id: row.id,
...
/**
 * Paginated Subprojects for one parent Project — Phase 1B+.
 */
...
export async function countCrmProjectsListV2(
  ...
  const totalCount = await callRootCountRpc(context.supabase, {
    organizationId: context.organizationId,
    userId: context.userId,
    visibility,
    request: context.request,
