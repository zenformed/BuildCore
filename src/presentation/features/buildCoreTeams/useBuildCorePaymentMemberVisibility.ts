...
import {
  fetchBuildCorePaymentMemberVisibilityBff,
  patchBuildCorePaymentMemberVisibilityBff,
} from '@/infrastructure/coreApi/buildCorePaymentMemberVisibilityBff';
...
export function useBuildCorePaymentMemberVisibility(enabled: boolean): {
  ...
  const load = useCallback(
    async (options?: { background?: boolean }) => {
      ...
      try {
        const next = await fetchBuildCorePaymentMemberVisibilityBff(token);
  ...
  const toggleOnlyAssignedUserCanView = useCallback(
    async (nextValue: boolean) => {
      ...
      try {
        await patchBuildCorePaymentMemberVisibilityBff(token, nextValue);
        const refreshed = await fetchBuildCorePaymentMemberVisibilityBff(token);
