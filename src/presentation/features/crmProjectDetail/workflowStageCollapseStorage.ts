...
/** projectSlug → stageSlug → expanded */
type PersistedMap = Record<string, Partial<Record<PipelineStageSlug, boolean>>>;
...
export function readWorkflowStageExpanded(
  ...
  const value = readAll()[projectSlug]?.[stageSlug];
  return value !== false;
}
...
export function writeWorkflowStageExpanded(
  ...
  const all = readAll();
  const project = { ...all[projectSlug], [stageSlug]: expanded };
