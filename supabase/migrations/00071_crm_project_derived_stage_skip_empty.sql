-- BuildCore: derive current workflow stage from ops tasks.
-- Empty stages remain on the roadmap as the current stage (no manual completion required).
-- Project Status → Completed confirmation ignores empty stages separately (task-count based).
-- Non-destructive: replaces function body only.
...
begin
  ...
  loop
    ...
    if v_task_count = 0 then
      -- Empty stage is still the current roadmap stage (visible pipeline catalog).
      return v_slug;
    end if;
