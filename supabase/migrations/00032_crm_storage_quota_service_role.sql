-- Allow ZenformedCore service-role uploads (customer portal, platform paths) to reserve/release quota
-- without an authenticated user JWT. Membership is enforced on staff routes via auth.uid().
