/**
 * GET/PATCH /api/internal/organization/payment-member-visibility
 * BuildCore-only setting for member payment milestone visibility (not ForgeCore relay).
 */
...
import {
  loadBuildCorePaymentVisibilitySettings,
  saveBuildCorePaymentVisibilitySettings,
} from '@/infrastructure/crm/server/buildCorePaymentVisibilityService';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const settings = await loadBuildCorePaymentVisibilitySettings(
      auth.context.supabase,
      auth.context.organizationId
    );
    return NextResponse.json(
    ...
    const message =
      err instanceof Error ? err.message : 'Could not load payment visibility settings.';
...
export async function PATCH(request: NextRequest): Promise<NextResponse> {
  if (runtimeModes.useMockAuth()) {
    return NextResponse.json(
      { onlyAssignedUserCanView: DEFAULT_BUILDCORE_PAYMENT_ONLY_ASSIGNED_USER_CAN_VIEW, canEdit: true },
      { headers: BUILDCORE_ADMIN_NO_CACHE_HEADERS }
    );
  }
  ...
  try {
    const saved = await saveBuildCorePaymentVisibilitySettings(
      auth.context.supabase,
      auth.context.organizationId,
      {
    ...
    const message =
      err instanceof Error ? err.message : 'Could not save payment visibility settings.';
    return NextResponse.json({ error: 'internal_error', message }, { status: 500 });
  }
}
